...
class PDF_General(object):
    def __init__(self, input_file_path: str):
        self.input_file_path = input_file_path
        self.flag = True
        self.file_id = '.'.join(input_file_path.split('/')[-1].split('.')[:2]).split('v')[0]
        ...
        try:
            ...
            self.flag = False
        self.sen_tokenizer = nltk.data.load('tokenizers/punkt/english.pickle')
        self.paper_type = None

        # self.most_width = None
        self.height = None
        self.width = None
    ...
    def extract_information_from_first_page(self, pages):
        """
        用前两页的文本判断论文类型
        -1: 论文中无法解析出text_box
        0：只能获取到正文的size和font
    ...
    def convert(self):
        ...
        try:
            pages = list()
            for i, page in zip(range(2), extract_pages(self.input_file_path)):
                pages.append(page)
            first_page = pages[0]
            self.height = first_page.bbox[3]
            self.width = first_page.bbox[2]
            paper_type = self.extract_information_from_first_page(pages)
            self.paper_type = paper_type
            if self.paper_type == -1:
                self.flag = False
                print(f"can't get text_box in file {self.input_file_path}")
                return
            if self.paper_type == 0 or self.paper_type == 1:
                self._convert_type_0()
            if self.paper_type == 2 or self.paper_type == 3 or self.paper_type == 4:
    ...
    def _convert_type_0(self):
        """
        无法获取标题信息的，只能按照正文字体和字号进行过滤，获得纯文本信息
        :return:
        """
        self.main_flag = True
        cur_boxes = list()
        for page in self.pages:
            ...
            for text_box in page:
                if isinstance(text_box, LTTextBox):
                    ...
                    if not self.reference_flag and len(text_box) == 1 and line_text.lower() in {"reference\n", "references\n"}:
                        self.reference_flag = True
                        self.sections["text"] = self._sort_boxes(cur_boxes)
                        cur_boxes = [[]]
                        self.main_flag = False
                        continue

                    if not self.appendix_flag and len(text_box) == 1 and line_text.lower().startswith("appendix") and len(line_text) < 100:
                    ...
                    if self.appendix_flag:
                        cur_font, cur_size = self._get_size_and_font(line)
                        if cur_font == self.text_font and cur_size == self.text_size:
                            cur_boxes[-1].append((text_box, (int(text_box.bbox[0]), int(text_box.bbox[2]), int(self.height - text_box.bbox[3]), int(self.height - text_box.bbox[1]))))
        ...
        if self.main_flag:
            ...
            self.appendix = self._sort_boxes(cur_boxes)
                ...
                if isinstance(text_box, LTTextBox):
                    ...
                    if not self.main_flag:
                        ...
                        if (result or line_text.endswith("introduction")) and len(line_text) <= 50:
                            ...
                            self.sections[self.cur_section] = list()
                            continue

                    if not self.reference_flag and len(text_box) == 1 and line_text.lower().strip() in {"reference", "references"}:
                        self.reference_flag = True

                        if self.cur_section:
                            self.sections[self.cur_section] = self._sort_boxes(cur_boxes)
                        cur_boxes = [[]]
                        self.main_flag = False
                        continue
                    ...
                    if self.main_flag:
                        ...
                        if cur_font == self.text_font:
                            cur_boxes[-1].append((text_box, (int(text_box.bbox[0]), int(text_box.bbox[2]), int(self.height - text_box.bbox[3]), int(self.height - text_box.bbox[1]))))
                        # else:
                        #     print(self._get_size_and_font(line), text_box)
                    if self.reference_flag:
                        cur_boxes[-1].append((text_box, (int(text_box.bbox[0]), int(text_box.bbox[2]), int(self.height - text_box.bbox[3]), int(self.height - text_box.bbox[1]))))
    ...
    def _clean_text(self, line):
        result = ""
        sentences = self.sen_tokenizer.tokenize(line)
        print(sentences)
    ...
    def combine(self):
        if self.paper_type == 0 or self.paper_type == 1:
            section = dict()
            section["key"] = "text"
            section["text"] = self._merge_paragraph(self.sections["text"])
            self.main_text.append(section)
            # self.sections["text"] = self._merge_paragraph(self.sections["text"])
        else:
            for key, value in self.sections.items():
                section = dict()
                section["key"] = key
                section["text"] = self._merge_paragraph(value)
                self.main_text.append(section)
                # self.sections[key] = self._merge_paragraph(value)
        if self.reference_flag:
            self.references = self._merge_references(self.references)
        if self.appendix_flag:
            self.appendix = self._merge_paragraph(self.appendix)
